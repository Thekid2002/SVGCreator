...
public interface ISvgDocument
{
    ...
    SvgDocument AddText(string content, (int x, int y) topLeft, int fontSize, string? fontFamily, int? rotation, Color? fillColor, Color? strokeColor, int? strokeWidth, string? additionalAttributesString = null);
    ...
    SvgDocument AddRectangle((int x, int y) topLeft, int width, int height, Color? fillColor, Color? strokeColor, int? strokeWidth, string? additionalAttributesString = null);
    string ToString();
}